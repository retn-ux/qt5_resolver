...
//该结构包含有关进程的已加载模块的信息
//PEB_LDR_DATA的InMemoryOrderModuleList每一项都指向这个结构
...
struct _LDR_DATA_TABLE_ENTRY
{
	...
	ULONG TimeDateStamp;
	TPVOID EntryPointActivationContext;		//指向_ACTIVATION_CONTEXT
	TPVOID Lock;
	TPVOID DdagNode;									//指向 _LDR_DDAG_NODE
	TLIST_ENTRY NodeModuleLink;
	TPVOID LoadContext;							//指向 _LDRP_LOAD_CONTEXT
...
struct _PEB // 65 elements, 0x210 bytes
{
	...
	TPVOID pFastPebLock;					//指向_RTL_CRITICAL_SECTION
	TPVOID AtlThunkSListPtr;				//指向_SLIST_HEADER		
	TPVOID IFEOKey;
	...
	DWORD dwAtlThunkSListPtr32;	//32位系统上指向_SLIST_HEADER
	TPVOID ApiSetMap;
	TDWORDPAD dwTlsExpansionCounter;//TLS索引计数
	TPVOID lpTlsBitmap;//TLS位图
	DWORD dwTlsBitmapBits[2];//动态Tls进程标志位
	...
	TPVOID lpLoaderLock;		//指向 _RTL_CRITICAL_SECTION
...

{
	...
	TDWORD ExitStatus; // 接收进程终止状态

	TDWORD PebBaseAddress; // 接收进程环境块地址

	TDWORD AffinityMask; // 接收进程关联掩码

	TDWORD BasePriority; // 接收进程的优先级类

	TDWORD UniqueProcessId; // 接收进程ID

	TDWORD InheritedFromUniqueProcessId; //接收父进程ID
...
typedef struct _KSERVER_TABLE_DESCRIPTOR {
	PULONG Base;	//表基址 指向KeServiceTable
	PULONG Count;
	ULONG Limit;	//总个数
	PUCHAR Number; //参数个数表
...
typedef struct _KSERVER_TABLE_DESCRIPTOR_SHADOW{
	//这里与KeServiceDescriptorTable完全一样
	PULONG Base;	//表基址 指向KeServiceTable
	PULONG Count;
	ULONG Limit;	//总个数
	PUCHAR Number; //参数个数表
	//这里时Shadow的独有表项
	PULONG BaseOfShadow;	//表基址 指向W32pServiceTable
	PULONG CountOfShadow;
	ULONG LimitOfShadow;	//总个数
	PUCHAR NumberOfShadow; //参数个数表
...
struct __TEB {
	...
	PVOID TlsSlots[TLS_MINIMUM_AVAILABLE];  //指向存放动态TLS数据的TLS Slot数组
	LIST_ENTRY TlsLinks;
	PVOID Vdm;
